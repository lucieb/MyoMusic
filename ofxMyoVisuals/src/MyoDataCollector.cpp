...
myo::Myo* MyoDataCollector::getMyo(int id) {
    if (id>=0 && id<knownMyos.size())
        return knownMyos[id];
    else
        return NULL;
}

MyoData* MyoDataCollector::getMyoData(myo::Myo *myo) {
    return &knownMyosData[myo];
}
...
void MyoDataCollector::onPair(myo::Myo* myo, uint64_t timestamp, myo::FirmwareVersion firmwareVersion)
{
    ...
    data.onArm = false;
    data.whichArm = myo::armUnknown;
    
    data.lastPose = myo::Pose::unknown;
    data.currentPose = myo::Pose::unknown;
...
{
    for (int m=0 ; m<knownMyos.size() ; m++) {
        
        MyoData &data = knownMyosData[knownMyos[m]];
        
        // Clear the current line
        std::cout << '\r';
        
        // Print out the orientation. Orientation data is always available, even if no arm is currently recognized.
        std::cout << (int)data.roll << '[' << std::string((int)data.roll/10, '*') << std::string(10 - (int)data.roll/10, ' ') << ']'
        << (int)data.pitch << '[' << std::string((int)data.pitch/10, '*') << std::string(10 - (int)data.pitch/10, ' ') << ']'
        << (int)data.yaw << '[' << std::string((int)data.yaw/10, '*') << std::string(10 - (int)data.yaw/10, ' ') << ']';
        ...
        if (data.onArm) {
            ...
            std::string poseString = data.currentPose.toString();
            
            std::cout << '[' << (data.whichArm == myo::armLeft ? "L" : "R") << ']'
